...
| **`supplier_url`** | ✅ Extracted from `a[href*="/users/profile/"]` near the host card (lands in DB column + raw_extras). Click-through to the host's profile page. |
| **Cover image (`first_image`)** | ✅ Prefers `img.pb-image-grid__img`; filter excludes `/airbnb-platform-assets/`, `/icons/`, `.svg` to avoid platform icons being picked as the hero |
| Packages (1 synthesized) | ✅ |
| Sections (15–20 captured, full {title, content}) | ✅ Stored in `raw_extras_json.sections` (capped 2KB per section) alongside flat `section_titles`. Lets the dashboard modal render bodies for "Things to know", category breakdowns, etc. |
