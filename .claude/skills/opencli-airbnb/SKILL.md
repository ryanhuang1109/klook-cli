...
- **Calendar nav has never produced rows** *(open, root cause unconfirmed, observed 2026-04-27)*: every airbnb pricing snapshot since the scaffold lands with `rows: []` and `errors: [{reason: 'cal-nav-failed: no-month-label'}, ...]`. Diagnostic instrumentation (committed) shows `headers=[] cells=[]` after `OPEN_PICKER_JS` returns ok — the picker click "succeeds" but the calendar widget never reaches the DOM tree we query. Reproducible on multiple experiences (`121104`, `4613909`, `555953`, `5814038`) under a zh-TW Browser Bridge cookie. Locale-aware month parsing has been added (CJK numeric "5 月" / "5월" + EN names) so that's no longer the suspect — calendar simply doesn't render. Likely causes, in priority order to investigate: (1) experience pages put the calendar inside a Shadow DOM or React portal we don't reach with `document.querySelector`; (2) airbnb requires a logged-in session to render the booking calendar (Browser Bridge cookie may not carry auth); (3) the picker selector clicks the wrong element (e.g. an FAQ "select date" link) without triggering calendar mount. Next step: open one of the validated experiences in a real Chrome window with the same Browser Bridge cookies, inspect what `OPEN_PICKER_JS` actually clicks, and check whether the calendar lives inside a Shadow root. Until then, **don't rely on airbnb pricing in the daily routine** — `ingested_pricing=N` from `tours ingest-listing` reports successful CALLS, not successful row capture.
