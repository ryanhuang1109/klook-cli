...
- **Adapter ignores `?lang=` query — non-English locales return 0 packages** *(open, observed 2026-04-28)*: the `tours` ingest pipeline now passes a per-listing `language` hint and `src/tours/url-locale.ts` rewrites the URL to e.g. `…?lang=pa`. GYG's web app DOES render different package sets per locale (verified on `t797962`: en → 3 packages, pa → 1), but `get-activity` and `get-pricing-matrix` both return empty arrays on those localised pages. Adapter selectors are written against the English DOM and the localised page either restructures the package list or hydrates it after a different interaction. Architecture-side merge (`mergePackageLanguages`) already accumulates `available_languages` across re-runs — the moment the adapter starts returning rows under non-en locales, multi-language ingest will Just Work end-to-end. Fix path: probe the localised pages first (`src/clis/getyourguide/probe.ts`), confirm whether package containers use the same selectors, then localise / interact accordingly.
